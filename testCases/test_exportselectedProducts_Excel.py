...
class Test_ExportSelectedProducts_Excel_047:
    ...
    def test_export_selected_products_excel(self, setup):

        self.logger.info(
            "******* Exporting Selected Products to Excel *******"
        )
        ...
        assert selected, (
            "Could not select any product for Excel export"
        )
        ...
        searchproduct.clickExportSelectedExcel()
        ...
        assert downloaded_file_path.lower().endswith(".xlsx"), (
            f"Unexpected downloaded file: "
            f"{downloaded_file_path}"
        )

        self.logger.info(
            "******* Selected Products Excel Export Passed *******"
        )

        print(
            "\nSelected product Excel export PASSED"
        )